...
export async function runAirprompt(
  args: string[],
  version: string,
): Promise<number> {
  ...
  try {
    ...
    if (options.dryRun) {
      info(
        'Dry run mode: airprompt will check your system but will not install or enable anything.',
      )
    }
    ...
    if (options.dryRun) {
      ...
      try {
        ...
        info(
          '(Tailscale not connected — connect to see your IP and final instructions)',
        )
      }
      return 0
    }
